...
<xsl:stylesheet version="1.0" xmlns:php="http://php.net/xsl"
	...
	<xsl:template match="tei:biblFull">
		...
		<xsl:variable name="proceedings"
			select="./tei:notesStmt/tei:note[@type='proceedings']/@n"/>
		<xsl:variable name='type_endnote'>
			<xsl:choose>
				<xsl:when test="$type='ART'">
					<xsl:text>Journal Article</xsl:text>
				</xsl:when>
				<xsl:when test="$type='REPORT' or $type='OTHERREPORT' or $type='REPACT'">
					<xsl:text>Report</xsl:text>
				</xsl:when>
				<xsl:when test="$type='COMM'">
					<xsl:choose>
					<xsl:when test="$proceedings=1">
				...
				<xsl:when test="$type='UNDEFINED' or $type='OTHER' or $type='MINUTES' or $type='NOTE' or $type='SYNTHESE' or $type='PRESCONF'">
		...
		<!-- pour les COMM sans actes, on ajoute %Z Oral -->
		<xsl:if test="$proceedings=0 and $type='COMM'">
			<xsl:text>&#xA;%F Oral </xsl:text>
		</xsl:if>
		<!-- pour les POSTER on ajoute %Z Oral -->
		...
		<xsl:apply-templates select="tei:seriesStmt/tei:idno"/>
	</xsl:template>
	...
	<xsl:template match="tei:note[@type='description']">
		...
		<xsl:choose>
			<xsl:when test="$type_endnote = 'Unpublished work'">
				<xsl:value-of select="concat('&#xA;%9 ',normalize-space(.))"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="concat('&#xA;%Z ',normalize-space(.))"/>
	...
	<xsl:template match="tei:note[@type='invited']">
		<xsl:if test="@n = 1">
			<text>&#xA;%F Invité</text>
		</xsl:if>
	</xsl:template>

	<xsl:template match="tei:note[@type='peer']">
		<xsl:choose>
			<xsl:when test="@n = 0">
				<text>&#xA;%&lt; sans comité de lecture</text>
			</xsl:when>
			<xsl:when test="@n = 1">
				<text>&#xA;%&lt; avec comité de lecture</text>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="tei:title" >
		...
		<xsl:choose>
			...
			<xsl:when test="@level='m'">
				<xsl:choose>
					<xsl:when test="$type_endnote = 'Book Section' or $type_endnote = 'Edited Book'">
						<xsl:value-of select="concat('&#xA;%B ',normalize-space(.))"/>
					</xsl:when>
					<xsl:when test="$type_endnote = 'Conference Proceedings'">
						<xsl:value-of select="concat('&#xA;%( ',normalize-space(.))"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat('&#xA;%3 ',normalize-space(.))"/>
	...
	<xsl:template match="tei:monogr" mode="scientificEditor">
		<xsl:param name="type_endnote"/>
		<xsl:variable name="codeEndnote">
			<xsl:choose>
				<xsl:when test="$type_endnote = 'Book Section'">
					<xsl:text>&#xA;%E </xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>&#xA;%Y </xsl:text>
		...
		<xsl:for-each select="tei:editor">
			<xsl:value-of select="concat($codeEndnote,normalize-space())"/>
		</xsl:for-each>
	</xsl:template>
	...
	<!-- authority -->
	<xsl:template match="tei:authority[@type='jury']">
		<xsl:value-of select="concat('&#xA;%Z ',normalize-space())"/>
	</xsl:template>
	<xsl:template match="tei:authority[@type='supervisor']">
		<xsl:value-of select="concat('&#xA;%Y ',normalize-space())"/>
	</xsl:template>
	...
	<!-- halId -->
	<xsl:template match="tei:idno[@type='halId']">
		<xsl:value-of select="concat('&#xA;%L ', normalize-space())"/>
	</xsl:template>	
	<!-- halUri -->
	...
	<!-- numero -->
	<xsl:template match="tei:idno[@type='patentNumber']|tei:idno[@type='reportNumber']|tei:idno[@type='nnt']">
		<xsl:value-of select="concat('&#xA;%N ', normalize-space())"/>
	</xsl:template>
	<!-- tampons -->
	<xsl:template match="tei:idno[@type='stamp']">
		<xsl:value-of select="concat('&#xA;%~ ', ./@n)"/>
	</xsl:template>
