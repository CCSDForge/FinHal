...
<!-- Generate bibtex from a TEI file A FAIRE : Title dans plusieurs langues ==> plusieurs TITLE (à changer) Keywords : pour le moment, tous les keywords de toutes les langues. A voir. Type de doc PRES_CONF, MEM -->
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="affAbstract" select="'no'"/>
	<xsl:variable name="smallcase" select="'abcdefghijklmnopqrstuvwxyzaeeei'" />
	<xsl:variable name="uppercase" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZàéèêî'" />
	...
	<xsl:template match="tei:biblFull">
		...
		<xsl:variable name="type_bibtex">
			<xsl:choose>
				...
				<xsl:when test="$type='DOUV'">
					<!--xsl:variable name="title" select="translate(./tei:titleStmt/tei:title, $uppercase, $smallcase)" /-->
					<!-- On prend tous les titres et pas seulement le 1er -->
					<xsl:variable name="AllTitlesCase">
					...
					<xsl:variable name="AllTitles" select="translate($AllTitlesCase, $uppercase, $smallcase)" />
					<xsl:variable name="title_vol" select="translate(./tei:sourceDesc/tei:biblStruct/tei:monogr/tei:imprint/tei:biblScope[@unit='serie'], $uppercase, $smallcase)" />
					<xsl:choose>
						<xsl:when test="contains($AllTitles,'proceeding') or contains($AllTitles,'actes') or contains($AllTitles,'conference') or contains($AllTitles,'conférence') or contains($AllTitles,'colloque') 
							or ./tei:sourceDesc/tei:biblStruct/tei:monogr/tei:meeting/tei:title !=''
							or contains($title_vol,'proceeding') or contains($title_vol,'actes') or contains($title_vol,'conference') or contains($title_vol,'conférence') or contains($title_vol,'colloque')" >
		...
		<!-- si type OTHER, on met la MD description dans le champ NOTE, sinon la MD  commentary-->
		<xsl:choose>
			<xsl:when test="$type='OTHER'">
				<xsl:apply-templates
					select="tei:notesStmt/tei:note[@type='description']" mode="note" />
			</xsl:when>
			<xsl:when test="$type='LECTURE' and string-length(tei:notesStmt/tei:note[@type='commentary'])!=0">
				<xsl:text>&#xA;  NOTE = {Lecture - </xsl:text>
				<xsl:value-of select="tei:notesStmt/tei:note[@type='commentary']"/>
				<xsl:text>},</xsl:text>
			</xsl:when>
			<xsl:when test="$type='LECTURE' and string-length(tei:notesStmt/tei:note[@type='commentary'])=0">
				<xsl:text>&#xA;  NOTE = {Lecture},</xsl:text>
			</xsl:when>
			<xsl:when test="$type='UNDEFINED' and string-length(tei:notesStmt/tei:note[@type='commentary'])=0">
				<xsl:text>&#xA;  NOTE = {working paper or preprint},</xsl:text>
			</xsl:when>
			<xsl:when test="$type='POSTER' and string-length(tei:notesStmt/tei:note[@type='commentary'])=0">
				<xsl:text>&#xA;  NOTE = {Poster},</xsl:text>
			</xsl:when>
			<xsl:when test="$type='POSTER' and string-length(tei:notesStmt/tei:note[@type='commentary'])!=0 and (contains(tei:notesStmt/tei:note[@type='commentary'],'poster') or contains(tei:notesStmt/tei:note[@type='commentary'],'Poster') or contains(tei:notesStmt/tei:note[@type='commentary'],'POSTER'))">
		...
		<!-- affichage de l'abstract si param affAbstract = yes -->
		<xsl:if test="$affAbstract='yes' ">
	...
	<xsl:template match="tei:monogr">
		...
		<!-- localreference -->
		<xsl:apply-templates select="./tei:idno[@type='localRef']" mode="localRef" />
	...
	<xsl:template match="tei:meeting">
		...
		<xsl:when test="$type_bibtex='proceedings' or $type_bibtex='inproceedings'">
			<xsl:if test="not(. = './tei:title') and string-length(normalize-space(./tei:title))!=0">
				<xsl:text>&#xA;  BOOKTITLE = {{</xsl:text>
				<xsl:call-template name="affiche_val">
...
<!-- localRef -->
<xsl:template match="tei:idno" mode="localRef">
	<xsl:text>&#xA;  HAL_LOCAL_REFERENCE = {</xsl:text>
	<xsl:call-template name="affiche_val">
		<xsl:with-param name="val"><xsl:value-of select="." /></xsl:with-param>
	</xsl:call-template>
	<xsl:text>},</xsl:text>
